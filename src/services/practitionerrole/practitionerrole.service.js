...
module.exports.count = (args) =>
  new Promise((resolve, reject) => {
    ...
    let collection = db.collection(`${COLLECTION.PRACTITIONERROLE}`);

    collection.countDocuments({}, (err, data) => {
      if (err) {
        logger.error('Error with PractitionerRole.count: ', err);
        return reject(err);
      }
      return resolve(data);
    });
...
module.exports.searchById = (args) =>
  new Promise((resolve, reject) => {
    ...
    collection.findOne({ id: id.toString() }, (err, practitionerrole) => {
      if (err) {
